...
export default function StoryDetailPage() {
  ...
  useEffect(() => {
    const storyData = getStoryById(params.id as string)
    if (storyData) {
      setStory(storyData)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-background flex items-center justify-center">
        <div className="text-center">
          <h2 className="font-serif text-2xl font-semibold mb-4">Story not found</h2>
          <Link href="/shelf">
            ...
            <CardHeader className="text-center pb-6">
              <div className="flex items-center justify-center gap-2 mb-4">
                <Badge variant="secondary" className="text-sm">
                  {story.template.charAt(0).toUpperCase() + story.template.slice(1)} Story
                </Badge>
              </div>
              <h1 className="story-title text-4xl md:text-5xl mb-6">{story.title}</h1>
              ...
              {/* Author Info (for community stories) */}
              {story.author && story.authorAvatar && (
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-muted-foreground">Story Creator</p>
                  </div>
                </div>
              )}
            </CardHeader>
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                ...
                {story.author && (
                  <Button
                    variant="outline"
                    onClick={handleLike}
                    className={`flex items-center gap-2 bg-transparent ${isLiked ? "text-red-500 border-red-200" : ""}`}
                  >
